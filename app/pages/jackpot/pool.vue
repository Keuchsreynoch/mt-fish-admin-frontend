  ...
  <div class="jackpot flex flex-col gap-3">
    ...
    <div class="jackpot-tabs">
      <div class="settings-flex-row mt-3">
        ...
        <JackpotGlobalConfigCard :pool-data="poolData" :update-loading="updateLoading" @submit="onSettingsSubmit"
          @cancel="fetchCurrentPool" />
        ...
        <v-card class="players-card" elevation="0">
          ...
          <div class="filters-row">
            <v-text-field v-model="memberSearch" density="compact" variant="outlined" hide-details clearable
              prepend-inner-icon="mdi-magnify" :placeholder="t('jackpot.searchMemberName')" class="member-search"
              @keyup.enter="onSearchSubmit" @click:clear="onSearchSubmit" />

            <v-btn color="success" variant="flat" class="refresh-mini-btn" :loading="playersLoading"
          ...
          <div class="table-scroll">
            <AppTable :columns="columnsReport" :items="reportData" :loading="playersLoading" :error="errorMessage"
              :page="currentPage" :page-size="itemsPerPage" :total-pages="totalReportPages" @update:page="onPageChange">
      ...
      <v-card class="reservations-jackpot-card mt-3" elevation="0">
        <div class="recent-winners">
          ...
          <div v-else class="recent-winners__grid">
            <div v-for="(history, idx) in sortedJackpotHistories" :key="`${history.id}-${history.payout_coin}-${idx}`"
              class="winner-chip">
